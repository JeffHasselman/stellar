import json
import os
from common import get_stellar _messages
import pytest
import requests
...
stellarGW_ENDPOINT = os.getenv(
    "stellarGW_ENDPOINT", "http://localhost:10000/v1/chat/completions"
)
...
def test_demos(test_data):
    ...
    response = requests.post(stellarGW_ENDPOINT, json=input)
    ...
    # now verify stellar _messages (tool call and api response) that are sent as response metadata
    stellar _messages = get_stellar _messages(response_json)
    assert len(stellar _messages) == 2
    tool_calls_message = stellar _messages[0]
