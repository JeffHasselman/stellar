...
async def test_function_calling(get_data_func):
    ...
    assert handler_map["stellar-Intent"].detect_intent(intent_response) == intent

    if intent:
        function_calling_response = await handler_map["stellar-Function"].chat_completion(
            req
        )
        assert (
        ...
        if parameter_gathering:
            prefill_prefix = handler_map["stellar-Function"].prefill_prefix
            assert any(
