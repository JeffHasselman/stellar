...
async def function_calling(req: ChatMessage, res: Response):
    ...
    try:
        ...
        intent_response = await handler_map["stellar-Intent"].chat_completion(req)
        intent_latency = time.perf_counter() - intent_start_time

        if handler_map["stellar-Intent"].detect_intent(intent_response):
            # TODO: measure agreement between intent detection and function calling
            try:
                function_start_time = time.perf_counter()
                final_response = await handler_map["stellar-Function"].chat_completion(req)
                ...
                final_response.metadata = {
                    ...
                    "hallucination": str(
                        handler_map["stellar-Function"].hallucination_state.hallucination
                    ),
                }
            except ValueError as e:
                res.statuscode = 503
                error_messages = f"[stellar-Function] - Error in tool call extraction: {e}"
            except StopIteration as e:
                res.statuscode = 500
                error_messages = f"[stellar-Function] - Error in hallucination check: {e}"
            except Exception as e:
                res.status_code = 500
                error_messages = f"[stellar-Function] - Error in ChatCompletion: {e}"
        else:
        ...
        res.status_code = 500
        error_messages = f"[stellar-Intent] - Error in ChatCompletion: {e}"
...
async def guardrails(req: GuardRequest, res: Response, max_num_words=300):
    ...
    try:
        ...
        final_response = handler_map["stellar-Guard"].predict(req)
        ...
        res.status_code = 500
        error_messages = f"[stellar-Guard]: {e}"
