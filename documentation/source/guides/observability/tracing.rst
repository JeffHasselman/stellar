...
and understanding complex AI agent stellar itectures like Co-pilots.

**stellar** propagates trace context using the W3C Trace Context standard, specifically through the
``traceparent`` header. This allows each component in the system to record its part of the request
flow, enabling **end-to-end tracing** across the entire application. By using OpenTelemetry, stellar ensures
...
1. **Enable Tracing Configuration**: Simply add the ``random_sampling`` in ``tracing`` section to 100`` flag to in the :ref:`listener <stellar _overview_listeners>` config

2. **Trace Context Propagation**: stellar automatically propagates the ``traceparent`` header. When a request is received, stellar will:
...
stellar uses the W3C Trace Context standard for trace propagation, which relies on the ``traceparent`` header.
...
below are very common practice, and not unique to stellar, when you reading tracing headers and export
...
::
    ...
    +--------------------------+
               ...
               v
    +--------------------------+        +--------------------------+
    |  Agent 1 (Main - stellar)   | ---->  | External Payment Service |
    +--------------------------+        +--------------------------+
               |                                  |
               v                                  v
    +--------------------------+        +--------------------------+
    |  Agent 2 (Support - stellar)| ---->  |   Internal Tech Support  |
    +--------------------------+        +--------------------------+
               |                                  |
               v                                  v
    +--------------------------+        +--------------------------+
    | Agent 3 (Orders- stellar)   | ---->  |   Inventory Management   |
    +--------------------------+        +--------------------------+
...
By leveraging the ``traceparent`` header for trace context propagation, stellar enables developers to implement
