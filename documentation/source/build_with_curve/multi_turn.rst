...
stellar is highly capable of accurately detecting and processing prompts in multi-turn scenarios so that you can buil fast and accurate agents in minutes.
Below are some cnversational examples that you can build via stellar. Each example is enriched with annotations (via ** [stellar] ** ) that illustrates how stellar
processess conversational messages on your behalf.

.. Note::
    The following section assumes that you have some knowledge about the core concepts of stellar, such as :ref:`prompt_targets <stellar _overview_prompt_handling>`.
    If you haven't familizaried yourself with stellar's concepts, we recommend you first read the :ref:`tech overview <tech_overview>` section firtst.
    Additionally, the conversation examples below assume the usage of the following :ref:`stellar_config.yaml <multi_turn_subsection_prompt_target>` file.
...
.. code-block:: text
    ...
    **[stellar]**: Check if there is an available <prompt_target> that can handle this user query.
    **[stellar]**: Found "get_info_for_energy_source" prompt_target in stellar_config.yaml. Forward prompt to the endpoint configured in "get_info_for_energy_source"
    ...
    ...
    **[stellar]**: Follow-up detected. Forward prompt history to the "get_info_for_energy_source" prompt_target and post the following parameters consideration="cost"
    ...
...
.. code-block:: text
    ...
    **[stellar]**: Check if there is an available <prompt_target> that can handle this user query.
    **[stellar]**: Found "diseases_symptoms" prompt_target in stellar_config.yaml. Forward disease=diabeteres to "diseases_symptoms" prompt target
    ...
    Assistant: Common symptoms include frequent urination, excessive thirst, fatigue, and blurry vision.

    User: How is it diagnosed?
    **[stellar]**: New intent detected.
    **[stellar]**: Found "disease_diagnoses" prompt_target in stellar_config.yaml. Forward disease=diabeteres to "disease_diagnoses" prompt target
    ...
...
--------------------------
The following section describes how you can easilly add support for multi-turn scenarios via stellar. You process and manage multi-turn prompts
just like you manage single-turn ones. stellar handles the conpleixity of detecting the correct intent based on the last user prompt and
...
For your convenience, we've built a `demo app <https://github.com/stellarlaboratory/stellar/main/demos/multi_turn_rag_agent>`_
