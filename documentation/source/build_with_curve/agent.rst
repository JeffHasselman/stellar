.. _stellar _agent_guide:

Agentic Apps
=============

stellar helps you build personalized agentic applications by calling application-specific (API) functions via user prompts.
...
stellar analyzes prompts, extracts critical information from prompts, engages in lightweight conversation with the user to
gather any missing parameters and makes API calls so that you can focus on writing business logic. stellar does this via its
purpose-built `stellar-Function <https://huggingface.co/collections/stellarlaboratory/stellar-function-66f209a693ea8df14317ad68>`_ -
...
--------------------
In the most common scenario, users will request a single action via prompts, and stellar efficiently processes the
request by extracting relevant parameters, validating the input, and calling the designated function or API. Here
is how you would go about enabling this scenario with stellar:
...
simultaneously or sequentially. With stellar, you can handle these scenarios efficiently using parallel or multiple
...
stellar-FC1B is built to manage these parallel tasks efficiently, ensuring low latency and high throughput, even
