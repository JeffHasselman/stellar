...
Below we describe the events in the lifecycle of a request passing through an stellar gateway instance. We first
describe how stellar fits into the request path and then the internal events that take place following
the arrival of a request at stellar from downtream clients. We follow the request until the corresponding
...
We recommend that you get familiar with some of the :ref:`terminology <stellar _terminology>` used in stellar
before reading this section.

Network topology
----------------

How a request flows through the components in a network (including stellar) depends on the network’s topology.
stellar can be used in a wide variety of networking topologies. We focus on the inner operation of stellar below,
but briefly we address how stellar relates to the rest of the network in this section.

- **Downstream(Ingress)** listeners take requests from upstream clients like a web UI or clients that forward
  prompts to you local application responses from the application flow back through stellar to the downstream.
...
In practice, stellar can be deployed on the edge and as an internal load balancer between AI agents. A request path may
traverse multiple stellar gateways:
...
High level stellar itecture
-----------------------
stellar is a set of **two** self-contained processes that are designed to run alongside your application servers
...
header sanitization, etc.), and the other process is for **model serving**, which helps stellar make intelligent
...
The request processing path in stellar has three main parts:

* :ref:`Listener subsystem <stellar _overview_listeners>` which handles **downstream** and **upstream** request
...
* :ref:`Prompt handler subsystem <stellar _overview_prompt_handling>` which is responsible for selecting and
...
* :ref:`Model serving subsystem <model_serving>` which helps stellar make intelligent decisions about the
  incoming prompts. The model server is designed to call the purpose-built LLMs in stellar.

The three subsystems are bridged with either the HTTP router filter, and the cluster manager subsystems of Envoy.

Also, stellar utilizes `Envoy event-based thread model <https://blog.envoyproxy.io/envoy-threading-model-a8d44b922310>`_.
A main thread is responsible forthe server lifecycle, configuration processing, stats, etc. and some number of
:ref:`worker threads <stellar _overview_threading>` process requests. All threads operate around an event loop (`libevent <https://libevent.org/>`_)
...
1. **TCP Connection Establishment**:
   A TCP connection from downstream is accepted by an stellar listener running on a worker thread.
...
2. **Prompt Guardrails Check**:
   stellar first checks the incoming prompts for guardrails such as jailbreak attempts. This ensures
   that harmful or unwanted behaviors are detected early in the request processing pipeline.

3. **Intent Matching**:
   The decrypted data stream is deframed by the HTTP/2 codec in stellar's HTTP connection manager. stellar performs
...
4. **Parameter Gathering with stellar-Function**:
   If a prompt target requires specific parameters, stellar engages stellar-FC to extract the necessary details
...
5. **API Call Execution**:
   stellar routes the prompt to the appropriate backend API or function call. If an endpoint cluster is identified,
...
A brief outline of the lifecycle of a request and response in the context of egress traffic from an application to Large Language Models (LLMs) via stellar:

1. **HTTP Connection Establishment to LLM**:
   stellar initiates an HTTP connection to the upstream LLM service. This connection is handled by stellar’s egress listener
...
2. **Rate Limiting**:
   Before sending the request to the LLM, stellar applies rate-limiting policies to ensure that the upstream LLM service
   is not overwhelmed by excessive traffic. Rate limits are enforced per client or service, ensuring fair usage and
   preventing accidental or malicious overload. If the rate limit is exceeded, stellar may return an appropriate HTTP
   error (e.g., 429 Too Many Requests) without sending the prompt to the LLM.

3. **Load Balancing to (hosted) LLM Endpoints**:
   After passing the rate-limiting checks, stellar routes the prompt to the appropriate LLM endpoint.
   If multiple LLM providers instances are available, load balancing is performed to distribute traffic evenly
   across the instances. stellar checks the health of the LLM endpoints using circuit breakers and health checks,
   ensuring that the prompt is only routed to a healthy, responsive instance.

4. **Response Reception and Forwarding**:
   Once the LLM processes the prompt, stellar receives the response from the LLM service. The response is typically a
   generated text, completion, or summarization. Upon reception, stellar decrypts (if necessary) and handles the response,
...
* The post-request :ref:`monitoring <monitoring>` are updated (e.g. timing, active requests, upgrades, health checks).
  ...
  thread periodically.
* :ref:`Access logs <stellar _access_logging>` are written to the access log
* :ref:`Trace <stellar _overview_tracing>` spans are finalized. If our example request was traced, a
