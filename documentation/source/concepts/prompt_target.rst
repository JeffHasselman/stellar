...
**Prompt Targets** are a fundamental component of stellar, enabling developers to define how different types of user prompts are processed and routed within their generative AI applications.
...
------------------------
Prompt targets are predefined endpoints within stellar that handle specific types of user prompts.
They act as the bridge between user inputs and your backend services or APIs, enabling stellar to route, process, and manage prompts efficiently.
...
--------------------------
Configuring prompt targets involves defining them in stellar's configuration file.
...
Defining Parameters
~~~~~~~~~~~~~~~~~~~
Parameters are the pieces of information that stellar needs to extract from the user's prompt to perform the desired action.
Each parameter can be marked as required or optional.
Here is a full list of parameter attributes that stellar can support:
...
stellar uses intelligent routing logic to ensure that prompts are directed to the appropriate targets based on their intent and context.

Default Targets
~~~~~~~~~~~~~~~
For general-purpose prompts that do not match any specific prompt target, stellar routes them to a designated default target.
This is useful for handling open-ended queries like document summarization or information extraction.

Intent Matching
~~~~~~~~~~~~~~~
stellar analyzes the user's prompt to determine its intent and matches it with the most suitable prompt target based on the name and description defined in the configuration.
...
Summary
--------
Prompt targets are essential for defining how user prompts are handled within your generative AI applications using stellar.
...
This modular approach not only simplifies your application's stellar itecture but also enhances scalability, maintainability, and overall user experience.
