...
def test_prompt_gateway(stream):
    ...
    if stream:
        ...
        assert response_json.get("model").startswith("stellar")
...
def test_prompt_gateway_stellar _direct_response(stream):
    body = {
        "messages": [
            {
                "role": "user",
    ...
    if stream:
        ...
        assert len(chunks) > 0
        response_json = json.loads(chunks[0])
        # make sure stellar  responded directly
        assert response_json.get("model").startswith("stellar")
