...
use common::consts::{
    stellar_PROVIDER_HINT_HEADER, stellar_ROUTING_HEADER, CHAT_COMPLETIONS_PATH,
...
impl StreamContext {
    ...
    fn select_llm_provider(&mut self) {
        let provider_hint = self
            .get_http_request_header(stellar_PROVIDER_HINT_HEADER)
...
impl HttpContext for StreamContext {
    ...
    fn on_http_request_headers(&mut self, _num_headers: usize, _end_of_stream: bool) -> Action {
        ...
        if self.llm_provider().endpoint.is_none() {
            self.add_http_request_header(
                stellar_ROUTING_HEADER,
                &self.llm_provider().provider_interface.to_string(),
            );
        } else {
            self.add_http_request_header(stellar_ROUTING_HEADER, &self.llm_provider().name);
        }
    ...
    fn on_http_response_body(&mut self, body_size: usize, end_of_stream: bool) -> Action {
        ...
        if self.streaming_response {
            ...
            //filed issue https://github.com/stellarlaboratory/stellar/issues/222
