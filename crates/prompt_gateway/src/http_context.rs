...
use common::{
    api::open_ai::{
        self, stellarState, ChatCompletionStreamResponse, ChatCompletionTool, ChatCompletionsRequest,
    },
    consts::{
        stellar_FC_MODEL_NAME, stellar_INTERNAL_CLUSTER_NAME, stellar_STATE_HEADER,
        stellar_UPSTREAM_HOST_HEADER, ASSISTANT_ROLE, CHAT_COMPLETIONS_PATH, HEALTHZ_PATH,
...
impl HttpContext for StreamContext {
    ...
    fn on_http_request_body(&mut self, body_size: usize, end_of_stream: bool) -> Action {
        ...
        self.stellar _state = match deserialized_body.metadata {
            Some(ref metadata) => {
                if metadata.contains_key(stellar_STATE_HEADER) {
                    let stellar _state_str = metadata[stellar_STATE_HEADER].clone();
                    let stellar _state: Vec<stellarState> = serde_json::from_str(&stellar _state_str).unwrap();
                    Some(stellar _state)
                } else {
                    None
                }
        ...
        let stellar _fc_chat_completion_request = ChatCompletionsRequest {
        ...
        let json_data = match serde_json::to_string(&stellar _fc_chat_completion_request) {
        ...
        debug!("stellar => stellar fc: {}", json_data);

        let mut headers = vec![
            (stellar_UPSTREAM_HOST_HEADER, MODEL_SERVER_NAME),
            (":method", "POST"),
        ...
        let call_context = StreamCallContext {
            response_handler_type: ResponseHandlerType::stellarFC,
            ...
            upstream_cluster: Some(stellar_INTERNAL_CLUSTER_NAME.to_string()),
    ...
    fn on_http_response_body(&mut self, body_size: usize, end_of_stream: bool) -> Action {
        ...
        if self.streaming_response {
            ...
            if self.tool_calls.is_some() && !self.tool_calls.as_ref().unwrap().is_empty() {
                let chunks = vec![
                    ChatCompletionStreamResponse::new(
                        ...
                        Some(stellar_FC_MODEL_NAME.to_string()),
            ...
            if !tool_calls.is_empty() {
                if self.stellar _state.is_none() {
                    self.stellar _state = Some(Vec::new());
                }
                ...
                if let Value::Object(ref mut map) = data {
                    // serialize stellar  state and add to metadata
                    let metadata = map
                        .entry("metadata")
                    ...
                    let stellar _state = HashMap::from([("messages".to_string(), fc_messages_str)]);
                    let stellar _state_str = serde_json::to_string(&stellar _state).unwrap();
                    metadata.as_object_mut().unwrap().insert(
                        stellar_STATE_HEADER.to_string(),
                        serde_json::Value::String(stellar _state_str),
                    );
                    let data_serialized = serde_json::to_string(&data).unwrap();
                    debug!("stellar <= developer: {}", data_serialized);
