...
use common::api::open_ai::{
    to_server_events, stellarState, ChatCompletionStreamResponse, ChatCompletionsRequest,
    ChatCompletionsResponse, Message, ModelServerResponse, ToolCall,
};
use common::configuration::{Overrides, PromptTarget, Tracing};
use common::consts::{
    stellar_FC_MODEL_NAME, stellar_FC_REQUEST_TIMEOUT_MS, stellar_INTERNAL_CLUSTER_NAME,
    stellar_UPSTREAM_HOST_HEADER, ASSISTANT_ROLE, MESSAGES_KEY, REQUEST_ID_HEADER, SYSTEM_ROLE,
...
pub struct StreamContext {
    ...
    pub stellar _state: Option<Vec<stellarState>>,
...
impl StreamContext {
    ...
    fn _trace_stellar _internal(&self) -> bool {
        match self._tracing.as_ref() {
            Some(tracing) => match tracing.trace_stellar _internal.as_ref() {
                Some(trace_stellar _internal) => *trace_stellar _internal,
                None => false,
            },
            None => false,
        }
    ...
    pub fn stellar _fc_response_handler(
        &mut self,
        body: Vec<u8>,
        mut callout_context: StreamCallContext,
    ) {
        let body_str = String::from_utf8(body).unwrap();
        debug!("stellar <= stellar fc response: {}", body_str);

        let server_response: ModelServerResponse = match serde_json::from_str(&body_str) {
            Ok(stellar _fc_response) => stellar _fc_response,
            Err(e) => {
                warn!(
                    "error deserializing stellar fc response: {}, body: {}",
                    e, body_str
                );
            ...
            ModelServerResponse::ModelServerErrorResponse(response) => {
                ...
                if response.result == "No intent matched" {
                        ...
                        .find(|pt| pt.default.unwrap_or(false))
                    {
                        ...
                        let stellar _messages_json = serde_json::to_string(&params).unwrap();
                        let timeout_str = stellar_FC_REQUEST_TIMEOUT_MS.to_string();

                        let mut headers = vec![
                            (":method", "POST"),
                            (stellar_UPSTREAM_HOST_HEADER, &upstream_endpoint),
                            (":path", &upstream_path),
                        ...
                        // if self.trace_stellar _internal() && self.traceparent.is_some() {
                        //     headers.push((TRACE_PARENT_HEADER, self.traceparent.as_ref().unwrap()));
                        // }

                        let call_args = CallArgs::new(
                            stellar_INTERNAL_CLUSTER_NAME,
                            &upstream_path,
                            headers,
                            Some(stellar _messages_json.as_bytes()),
                            vec![],
        ...
        stellar _fc_response.choices[0]
            .message
            .tool_calls
        ...
        if self.tool_calls.is_none() || self.tool_calls.as_ref().unwrap().is_empty() {
            // This means that stellar FC did not have enough information to resolve the function call
            // stellar FC probably responded with a message asking for more information.
            ...
            let direct_response_str = if self.streaming_response {
                let chunks = vec![
                    ChatCompletionStreamResponse::new(
                        ...
                        Some(stellar_FC_MODEL_NAME.to_owned()),
                        None,
                    ),
                    ChatCompletionStreamResponse::new(
                        Some(
    ...
    fn schedule_api_call_request(&mut self, mut callout_context: StreamCallContext) {
        ...
        let mut headers = vec![
            (stellar_UPSTREAM_HOST_HEADER, endpoint.name.as_str()),
            (":method", &http_method),
            (":path", &path),
        ...
        debug!(
            "stellar => api call, endpoint: {}{}, body: {}",
    ...
    pub fn api_call_response_handler(&mut self, body: Vec<u8>, callout_context: StreamCallContext) {
        ...
        debug!(
            "stellar <= api call response: {}",
            self.tool_call_response.as_ref().unwrap()
        );

        let mut messages = self.filter_out_stellar _messages(&callout_context);
        ...
        debug!("stellar => llm request: {}", llm_request_str);
    ...
    fn filter_out_stellar _messages(&mut self, callout_context: &StreamCallContext) -> Vec<Message> {
    ...
    pub fn generate_toll_call_message(&mut self) -> Message {
        Message {
            ...
            content: None,
            model: Some(stellar_FC_MODEL_NAME.to_string()),
    ...
    pub fn default_target_handler(&self, body: Vec<u8>, mut callout_context: StreamCallContext) {
        ...
        debug!("stellar => (default target) llm request: {}", json_resp);
