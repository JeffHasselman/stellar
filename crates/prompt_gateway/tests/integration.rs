...
{\
    ...
    module
        ...
        .expect_log(Some(LogLevel::Trace), None)
        .expect_http_call(
            Some("stellar _internal"),
            Some(vec![
                ("x-stellar -upstream", "server"),
                (":method", "POST"),
...
fn prompt_gateway_successful_request_to_open_ai_chat_completions() {
    ...
    module
        ...
        .expect_log(Some(LogLevel::Debug), None)
        .expect_log(Some(LogLevel::Trace), None)
        .expect_http_call(Some("stellar _internal"), None, None, None, None)
        .returning(Some(4))
        .expect_metric_increment("active_http_calls", 1)
        .execute_and_expect(ReturnType::Action(Action::Pause))
        .unwrap();
...
fn prompt_gateway_request_to_llm_gateway() {
    ...
    let stellar _fc_resp = ChatCompletionsResponse {
        usage: Some(Usage {
            completion_tokens: 0,
        }),
        choices: vec![Choice {
    ...
    let stellar _fc_resp_str = serde_json::to_string(&stellar _fc_resp).unwrap();
    module
        .call_proxy_on_http_call_response(http_context, 1, 0, stellar _fc_resp_str.len() as i32, 0)
        .expect_metric_increment("active_http_calls", -1)
        .expect_get_buffer_bytes(Some(BufferType::HttpCallResponseBody))
        .returning(Some(&stellar _fc_resp_str))
        .expect_log(Some(LogLevel::Debug), None)
        .expect_log(Some(LogLevel::Debug), None)
        .expect_log(Some(LogLevel::Debug), None)
        .expect_log(Some(LogLevel::Debug), None)
